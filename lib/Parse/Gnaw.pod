...
Only top level functions return grammar code references. They include:
	...
	match	Similar to perls =~ m/// regexp function.
		Find the first match of the grammar in the string.
		Skip any text until we find a match.

	swap	Similar to perls =~s/// regexp function.
		Define a pattern, and define either a replacement string
		or a subroutine which will receive the original text and
		return a replacement string. 

	parse	Similar to perls =~ m/\A// regexp function.
		Parse the string from the beginning only. 
		Do not scan through text looking for a match.

	modify	Allow grammars to modify text in flight.
		Note that the "modify" function does not modify anything
		unless the user calls the "get" function and defines 
		what gets modified. Think of "modify" as "match" but
		with the "write protect" bit turned off.
...
swap examples:
	...
	# swap 'Alice' for 'Bob', use string literal
	...
	# swap a phone number for the same number, but with asterisks around it.
	my $input = "please call 555-1212 for more details";
	swap(some(ccdigit),'-',some(ccdigit), sub{return "***".shift(@_)."***";}) -> ($input);
...
=head2 x (exchange inline)

The "x" function is used to exchange text in the string being parsed with
some other text supplied by the user. The user provides a subroutine reference
as the first parameter. If the grammar matches, the "x" function will take 
the text that matched, pass it into the users code reference, and then
whatever that subroutine call returns will replace the matching text.

Note that "x" is a lower level function, so it can be used within a grammar
just like "get" or "a" or "g" and so on.

The following example looks for 'hello', name, '!' and exchanges name with 
an all upper case version of name.
...
The word boundary position marker is also needed. It will match on four different
situations:
