...
and for those who are familiar with the perl regular expression 
convention of using the numbered variables to hold captured data, the
"c" function provides similar functionality to the capturing parens.

The "c" function is a wrapper around the "get" function.
The difference is that the first parameter of the "get" function
is removed in the "c" function. Internally the "c" function
passes into "get" a subroutine specific to "c" which assigns the
captured results to some Parse::Gnaw numbered variables. Since 
the numbered variables ($1,$2,$3,etc) are read only, the capture 
variables in Parse::Gnaw are similar, but with a tweak:
...
The number in the variable name is determined the same way that the
numbered variables are determined in perl regular expressions: by
counting the left hand parenthesis, and by counting from left to right.
...
If you think of the "c" function as capturing parens from perl regular
expressions, then the "c" function will capture data to the corresponding
variable.
